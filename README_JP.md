...
```sh
SEARXNG_BASE_URL="http://searxng:8080" # 値はバックエンドをどこで実行するかによって異なります — 下記のSEARXNGに関する注意を参照
SEARXNG_PORT=8080
...
- **SEARXNG_PORT**: DockerがSearXNGを公開する**ホスト**側のポート。マシン上でポート `8080` が既に使用されている場合は、別のポート（例：`8001`）を設定してください。Docker内部ではコンテナは常に `8080` でリッスンしており、この変数がそれを変えることはありません。
- **SEARXNG_BASE_URL**: **バックエンド**がSearXNGへアクセスするために使用するアドレス。ここ `.env` で設定します（`config.ini` ではありません）。値はバックエンドをどこで実行するかだけで決まります：

| AgenticSeekの実行方法 | `SEARXNG_BASE_URL` |
|---|---|
| Webインターフェース — バックエンドをDockerで実行（`./start_services.sh full`） | `http://searxng:8080` — `SEARXNG_PORT` を変更していても常にポート `8080` |
| CLIモード — バックエンドをホストで実行（`uv run cli.py`） | `http://localhost:8080` — `SEARXNG_PORT` を変更した場合はそのポートを使用してください（例：`http://localhost:8001`）。`http://searxng:...` はここでは**使えません**：このホスト名はDocker内部にしか存在しません |

> ブラウザでSearXNGを確認する場合は、常にホスト側のポート `http://localhost:<SEARXNG_PORT>` を使用してください。`.env` を変更した後はバックエンドを再起動してください — このファイルはプロセスの起動時にしか読み込まれません。
...
次に、`.env` ファイル（`config.ini` では**ありません**）のSEARXNG_BASE_URLをホスト側にマッピングされたアドレスに変更する必要があります。CLIモードではバックエンドがDockerの外、つまりあなたのマシン上で実行されるためです：

```sh
SEARXNG_BASE_URL="http://localhost:8080"
```

> `.env` で `SEARXNG_PORT` を変更した場合は、ここではそのポートを使用してください（例：`http://localhost:8001`）。`.env` を編集した後は `cli.py` を再起動してください — この値は起動時に読み込まれます。
...
`SEARXNG_BASE_URL` は `.env` で設定され、**バックエンドをどこで実行するか**だけで決まります：

**バックエンドをホストで実行（CLIモード）：** `SEARXNG_BASE_URL="http://localhost:8080"` — `SEARXNG_PORT` を変更した場合はそのポートを使用してください（例：`http://localhost:8001`）。`http://searxng:...` はここでは**使えません**：このホスト名はDocker内部でしか解決されません。

**バックエンドをDockerで実行（Webインターフェース、`full` プロファイル）：** `SEARXNG_BASE_URL="http://searxng:8080"` — `SEARXNG_PORT` を変更していても常にポート `8080` です。この変数はホスト側のポートを付け替えるだけです。

> **ポート競合に関する注意**: ホスト上でポート `8080` が既に使用されている場合は、`.env` で `SEARXNG_PORT`（例：`8001`）を設定し、その上で：
> - Webインターフェース（バックエンドをDockerで実行）：`SEARXNG_BASE_URL="http://searxng:8080"` のままにしてください — Docker内部のポートは変わりません。
> - CLIモード（バックエンドをホストで実行）：`SEARXNG_BASE_URL="http://localhost:8001"` に設定してください — `SEARXNG_PORT` と一致している必要があります。
> - ブラウザでの確認：`http://localhost:<SEARXNG_PORT>` を開いてください — 変更前のポート `8080` で応答するものがあれば、それはAgenticSeekのSearXNGではなく別のアプリケーションです。
>
> `.env` を変更した後はバックエンド（`api.py` または `cli.py`）を再起動してください — このファイルはプロセスの起動時にしか読み込まれません。
